...
namespace :aggregated_user do
  ...
  task :add, %i[id username] => :environment do |_t, args|
  ...
  task :add_and_backfill, %i[id username] => :environment do |_t, args|
    AddAggregatedUser.call(
      id: args[:id].to_i,
      username: args[:username],
      backfill: true
  ...
  task :remove_and_purge, [:username] => :environment do |_t, args|
    RemoveAggregatedUser.call(
      username: args[:username],
      purge: true
    )
